...
from matplotlib import pyplot, gridspec, path, patches, ticker
...
def plot_snow(HUC8, precipitation, temperature, simulated_snow, simulated_fall,
    ...
    avg_rain = sum(precipitation[1]) / (precipitation[0][-1].year -
                                        precipitation[0][0].year)
    avg_fall = sum(simulated_fall[1]) / (simulated_fall[0][-1].year -
                                         simulated_fall[0][0].year)
    ...
    if observed_fall is not None:
        ...
        avg = sum(fall) / (times[-1].year - times[0].year)
