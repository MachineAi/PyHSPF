...
class Postprocessor:
    ...
    def get_total_flow(self, comid, dates = None):
        ...
        #vols = self.get_reach_outvolume(comid, dates = dates)
    ...
    def get_sim_flow(self, comid, dates = None, tstep = 'daily'):
        ...
        #volumes = self.get_reach_outvolume(comid, dates = dates)
        times, vols = self.get_reach_timeseries('ROVOL', comid, 
                                                tstep = 'hourly', dates = dates)
    ...
    def get_storms(self, comid, upcomids = [], season = 'all', dates = None,
        ...
        for start, end in zip(years[:-1], years[1:]):
            ...
            if len([t for t in data[0] if t.year == start.year]) > 0:
                ...
                times, prec = self.get_precipitation(comid, tstep = 'daily',
                                                     dates = (start, end))
    ...
    def plot_snow(self, comid = None, tstep = 'daily', output = None, 
                  show = True):
    ...
    def plot_hydrograph(self, comid = None, tstep = 'daily', output = None, 
                        show = True):
        ...
        pet   = self.get_pet(comid = comid, tstep = tstep)
        evap  = self.get_evaporation(comid, tstep = tstep)
